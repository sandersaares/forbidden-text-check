...
use forbidden_text_check::{FORBIDDEN_TEXTS, is_forbidden_text_static};
...
fn main() {
    let Some(two_processors) = ProcessorSet::builder()
        ...
        eprintln!("This example requires the system to have at least two memory regions.");
        return;
    };

    let first = two_processors.processors().first().clone();
    let second = two_processors.processors().last().clone();

    thread::spawn(move || {
        println!(
            "Generating data set on processor {first} in memory region: {}",
            first.memory_region_id()
        );

        ProcessorSet::from_processor(first).pin_current_thread_to();

        // The black box ensures that the operation is not optimized away due to unused result.
        _ = black_box(FORBIDDEN_TEXTS.len());
    })
    .join()
    .unwrap();
    ...
    thread::spawn(move || {
        println!(
            "Checking data set on processor {second} in memory region: {}",
            second.memory_region_id()
        );

        ProcessorSet::from_processor(second).pin_current_thread_to();

        for _ in 0..ITERATION_COUNT {
            // The black box ensures that the operation is not optimized away due to unused result.
            black_box(is_forbidden_text_static(HAYSTACK));
        }
    })
    .join()
    .unwrap();
